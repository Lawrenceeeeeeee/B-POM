...
class SentimentModelTrainer:
    ...
    def load_and_split_data(self):
        ...
        df.dropna(subset=['content', 'index_manyi'], inplace=True)  # 删除任何含有NaN的行
        final_count = df.shape[0]
        df['index_manyi'] = df['index_manyi'].apply(lambda x: int(x) - 1)  # 转换标签

        print(f"Initial data rows: {initial_count}, After NaN removal: {final_count}")  # 输出数据条数

        texts = df['content'].tolist()
        labels = df['index_manyi'].tolist()
    ...
    def train(self, batch_size=8, epochs=5):
...
filepath = 'tag_data/traindata.csv'
trainer = SentimentModelTrainer(model_path, tokenizer, filepath)
trainer.train()
trainer.save_model('Bert-updated-manyi')
