...
class SentimentModelTrainer:
    ...
    def load_and_split_data(self):
        ...
        df.dropna(subset=['content', 'bool_tiwen'], inplace=True)  # 删除任何含有NaN的行
        final_count = df.shape[0]
        df['bool_tiwen'] = df['bool_tiwen'].apply(lambda x: int(x) - 1)  # 转换标签

        print(f"Initial data rows: {initial_count}, After NaN removal: {final_count}")  # 输出数据条数

        texts = df['content'].tolist()
        labels = df['bool_tiwen'].tolist()
...
trainer.train()
trainer.save_model('Bert-updated-tiwen')
