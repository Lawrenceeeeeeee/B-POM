...
class SentimentModelTrainer:
    ...
    def load_and_split_data(self):
        ...
        df.dropna(subset=['content', 'bool_taolun'], inplace=True)  # 删除任何含有NaN的行
        final_count = df.shape[0]
        df['bool_taolun'] = df['bool_taolun'].apply(lambda x: int(x) - 1)  # 转换标签

        print(f"Initial data rows: {initial_count}, After NaN removal: {final_count}")  # 输出数据条数

        texts = df['content'].tolist()
        labels = df['bool_taolun'].tolist()
...
trainer.train()
trainer.save_model('Bert-updated-taolun')
